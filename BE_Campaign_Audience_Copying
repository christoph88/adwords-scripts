function audienceCopy(account) {
  ...
  var campaignNameContains = ["_B_","_NB_"];
  ...
  var includeShoppingCampaigns = false;
  ...
  var campaignToCopy = "_B_Branding";
  ...
  function execute() {
    ...
    var templateCampaigns = AdWordsApp.campaigns()
      .withCondition('CampaignName CONTAINS "' + campaignToCopy.replace(/"/g,'\\\"') + '"')
...
function updateAccountsInSeries() {
  ...
  while (accountIterator.hasNext()) {
    ...
    // Switch to the account you want to process and pass this to the main function.
    audienceCopy(MccApp.select(account));
